...
class ColumnRangeOptions(ColumnBase):
    threshold = Integer(None, allow_none=True)
    turboThreshold = Integer(None, allow_none=True)
    minPointLength = Integer(None, allow_none=True)

    def __init__(self, **kwargs):
        super(__class__, self).__init__(**kwargs)         # noqa F821


class ColumnRange(ColumnRangeOptions):
    type_ = Unicode("columnrange", read_only=True)
